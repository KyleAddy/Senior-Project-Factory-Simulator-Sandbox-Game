...
public class playerCrafting : MonoBehaviour
{
    ...
    void CraftItem()
    {
        //finds an empty inv slot and fills it witht he item that is being crafted
        int index = GetComponent<gameGlobal>().findEmptArraySlot(GetComponent<Inventory>().inventory, GetComponent<Inventory>().invSize);
        if (index != -1)
